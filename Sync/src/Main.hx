		...
		for (id in updateVars)
		{
			...
			for (f in familias)
			{
				...
				for (m in moradores)
				{
					...
					shouldInsert = false;
					var old_morador = Morador.manager.unsafeObject("SELECT * FROM Morador WHERE old_id = " + m.old_id + " ORDER BY syncTimestamp DESC LIMIT 1", false);
					for (f1 in Morador.manager.dbInfos().fields)
					{
						var f = f1.name;
						if (f != "syncTimestamp" && f != "id" && Reflect.field(new_morador, f) != Reflect.field(old_morador, f))
							shouldInsert = true;
					}
				...
				while (i < points.length)
				{
					...
					{
						var f = f1.name;
						if (f != "session_id" && f != "id" && f != "syncTimestamp" && Reflect.field(new_point, f) != Reflect.field(old_point, f))
							shouldInsert = true;						
					}
					...
					for (mo in modos)
					{
						...
						{
							var f = f1.name;
							if (f != "id" && f != "syncTimestamp" && Reflect.field(new_modo, f) != Reflect.field(oldmodo, f))
								shouldInsert = true;							
						}
						if (shouldInsert)
							new_modo.insert();
