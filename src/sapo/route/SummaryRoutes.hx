...
class SummaryRoutes extends AccessControl
{
	...
	function statusGen() : Map<Int,Map<Int,PesqStatus>>
	{
		...
		var controlResults = Manager.cnx.request("SELECT s.user_id as user, s.`group` as grupo, COUNT(*) as pesqGrupo, SUM(CASE WHEN (checkSV IS NULL OR checkCT IS NULL OR checkCQ IS NULL) OR ((checkSV IS NOT NULL AND checkSV != 0 AND checkCT IS NOT NULL AND checkCT != 0 AND checkCQ IS NOT NULL AND checkCQ != 0) AND NOT (checkSV = 1  AND checkCT = 1 AND checkCQ = 1) ) THEN 1 ELSE 0 END) as Completa, SUM(CASE WHEN checkSV = 0 AND checkCT = 0 AND checkCQ = 0 THEN 1 ELSE 0 END) as allFalse, SUM(CASE WHEN checkSV = 0 OR checkCT = 0 OR checkCQ = 0 THEN 1 ELSE 0 END) as hasFalse, SUM(CASE WHEN checkSV = 1 AND checkCT = 1 AND checkCQ = 1 THEN 1 ELSE 0 END) as isTrue, SUM( CASE WHEN checkSV IS NULL THEN 1 ELSE 0 END) as nullSupervisor, SUM( CASE WHEN checkCT IS NULL THEN 1 ELSE 0 END) as nullCT, SUM(CASE WHEN checkCQ IS NULL THEN 1 ELSE 0 END) AS nullSuper FROM Survey s JOIN UpdatedSurvey us ON s.old_survey_id = us.old_survey_id AND s.syncTimestamp = us.syncTimestamp WHERE s.syncTimestamp > 1000 GROUP BY s.user_id, s.`group` ORDER BY s.user_id, s.`group`").results();
